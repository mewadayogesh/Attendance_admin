...
@app.route('/report/attendance/excel')
@login_required
def report_attendance_excel():
    """Export attendance report as Excel spreadsheet including Check Out times."""
    import pandas as pd

    conn = db.get_db()
    
    base_query = '''
        SELECT a.id, e.name AS employee_name, e.employee_id AS custom_emp_id, 
               a.work_date, a.check_in, a.check_out, a.status, a.created_at
        FROM attendance a
        LEFT JOIN employees e ON a.employee_id = e.id
    '''
